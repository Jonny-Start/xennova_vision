...
class SmartPlateDetector(IPlateDetector):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            print("💡 Instala dependencias: pip install easyocr pytesseract")
            self.is_initialized = False
            return False
    ...
    def _detect_plate_regions(self, image: np.ndarray) -> List[tuple]:
        ...
        try:
            ...
            # Si no encuentra regiones, usar imagen completa
            if not plate_regions:
                h, w = image.shape[:2]
                plate_regions.append((w//4, h//4, w//2, h//2))  # Centro de la imagen
    ...
    def _analyze_confidence(self, plate_text: str) -> tuple:
        ...
        if confirmations >= self.min_confirmations:
            if plate_text != self.last_confirmed_plate:
                ...
                print(f"🚀 RAPIDO! Placa confirmada: {plate_text}")
                return True, 0.95, f"RAPIDO - {plate_text}"
